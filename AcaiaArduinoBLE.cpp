...
bool AcaiaArduinoBLE::initNonBlocking(String mac)
{
    ...
    if (!_scanActive)
    {
        // Start or restart a scan when we don't have one running
        BLE.scan();
        _scanActive = true;
        _scanStart = millis();
    }
    ...
    bool nameMatches = peripheral && isScaleName(peripheral.localName());
    bool macMatches = peripheral && mac.length() && (peripheral.address() == mac);
    bool accept = peripheral && (mac.length() ? macMatches : nameMatches);
...
bool AcaiaArduinoBLE::init(String mac, unsigned long timeoutMs)
{

    Serial.print("AcaiaArduinoBLE Library v");
    Serial.print(LIBRARY_VERSION);
    Serial.println(" reinitializing (blocking)...");
...
bool AcaiaArduinoBLE::isScaleName(String name)
{
    ...
    return nameShort == "CINCO" || nameShort == "ACAIA" || nameShort == "PYXIS" || nameShort == "LUNAR" || nameShort == "PEARL" || nameShort == "PROCH" || nameShort == "BOOKO";
}
