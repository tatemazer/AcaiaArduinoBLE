...
public:
    ...
    // Blocking: scans up to timeoutMs (default 10s) before giving up
    bool init(String mac = "", unsigned long timeoutMs = 10000);
    // Non-blocking: starts/continues scanning and returns immediately; call each loop
    bool initNonBlocking(String mac = "");
    bool tare();
    bool startTimer();
    bool stopTimer();
    bool resetTimer();
